/* eslint-disable @next/next/no-img-element, unicorn/prefer-top-level-await -- Satori requires native images and Next's edge OG pattern keeps bundled font loading asynchronous. */
import ky from 'ky'
...
const interSemiBoldFontPromise = fetch(
...
export default async function OGImage(req: NextRequest) {
  ...
  const interSemiBoldFont = await interSemiBoldFontPromise
...
> {
  ...
  const isRootPage = uuidToId(block.id) === uuidToId(libConfig.rootNotionPageId)
  const isBlogPost =
    block.type === 'page' &&
    !isRootPage &&
    !isRootPageChild(block.id, recordMap, libConfig.rootNotionPageId)
  ...
  // )
  const publishedTime =
    getPageProperty<number>('Published', block, recordMap) || block.created_time
